...
<div class="total-container">
  ...
  <!-- Input field untuk menyimpan ID nota -->
  <input type="hidden" id="idNota" value="">
</div>

  <div class="center-container">
  <button class="checkout" onclick="checkout()">Bayar</button>
  <a class="home-button" href="generate_pdf.php?id_nota=<?php echo $idNota; ?>" target="_blank">Cetak Bukti Pembayaran</a>
</div>
  ...
  // Panggil fungsi setHapusBarangEventListener() saat halaman selesai dimuat
  window.addEventListener('load', setHapusBarangEventListener);

  // Fungsi untuk checkout dan menyimpan data ke database
function checkout() {
  const keranjang = document.querySelectorAll('#keranjang tr');
  const dataTransaksi = [];
  let totalHarga = 0; // Inisialisasi totalHarga dengan nilai 0
  let uangPembayaran = parseInt(document.getElementById('uangPembayaran').value); // Ambil nilai uangPembayaran
  ...
  keranjang.forEach(row => {
    ...
    const hargaJual = parseInt(row.querySelector('td:nth-child(3)').textContent); // Ambil hargaJual dari tabel

    dataTransaksi.push({ id_barang: idBarang, jumlah });
    totalHarga += hargaJual * parseInt(jumlah); // Hitung totalHarga
  });

  // Update nilai totalHarga pada input
  document.getElementById('totalHarga').value = totalHarga;

  if (!uangPembayaran || uangPembayaran < totalHarga) {
    alert('Uang pembayaran kurang!');
    return; // Mencegah eksekusi lebih lanjut jika pembayaran kurang
  }

  // Simpan dataTransaksi ke server untuk disimpan ke database (ganti dengan endpoint Anda)
  fetch('simpan_transaksi.php', {
    ...
    .then(response => response.json())
    .then(data => {
      // Simpan ID nota yang baru di-generate ke dalam input field tersembunyi
      document.getElementById('idNota').value = data.id_nota;
      // Tampilkan pesan berhasil
      alert('Transaksi berhasil! ID Nota: ' + data.id_nota);
      // Refresh halaman setelah transaksi berhasil
